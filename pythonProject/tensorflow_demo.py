...
global sex_var , chest_pain_var, age_entry,model,bp_entry,ch_entry,fbs_entry,ecg_var,mhx_entry,exa_var,op_var,sts_var,out_label
...
def run_form():
    ...
    chest_pain_frame.grid(row=2, column=1, sticky="w")
    ...
    ecg_label.grid(row=6, column=0, sticky="w")
    ecg_var = tk.StringVar(value="Normal")
    ecg_frame = tk.Frame(form_frame)
    ecg_frame.grid(row=6, column=1, sticky="w")
    ...
    mhx_label.grid(row=7, column=0, sticky='w')
    global mhx_entry
    mhx_entry = tk.Entry(form_frame)
    mhx_entry.grid(row=7, column=1)

    global exa_var
    exa_label = tk.Label(form_frame, text="Exercise Angina :")
    exa_label.grid(row=8, column=0, sticky="w")
    exa_var = tk.StringVar(value="N")
    exa_frame = tk.Frame(form_frame)
    exa_frame.grid(row=8, column=1, sticky="w")
    ...
    op_label.grid(row=9, column=0, sticky='w')
    global op_var
    op_var = tk.Entry(form_frame)
    op_var.grid(row=9, column=1)

    global sts_var
    sts_label = tk.Label(form_frame, text="ST Slope :")
    sts_label.grid(row=10, column=0, sticky="w")
    sts_var = tk.StringVar(value="Down")
    sts_frame = tk.Frame(form_frame)
    sts_frame.grid(row=10, column=1, sticky="w")
    ...
    out_label.grid(row=12,column=0)
